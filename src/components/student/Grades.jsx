...
const Grades = () => {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:5555/grades')
      ...
      .then(data => {
        // Access the grades array inside the response data
        ...
        <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
          <thead>
            <tr className="bg-gray-800 text-white">
              ...
              <th className="py-2 px-4 text-left">Class ID</th>
              <th className="py-2 px-4 text-left">Grade</th>
            </tr>
          </thead>
          <tbody>
            {grades.map((grade) => (
              <tr key={grade.id} className="border-b hover:bg-gray-50">
                <td className="py-2 px-4">{grade.course_id}</td>
                <td className="py-2 px-4">{grade.class_id}</td>
                <td className="py-2 px-4">{grade.grade}</td>
              </tr>
            ))}
          </tbody>
